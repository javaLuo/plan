      ...
      if (res) {
        ...
        for (let i = 0; i < dom_lis.length; i++) {
          ...
          <div>
            ...
            <button data-type="start" data-symbols="${temp[i].d1.symbol}@${temp[i].d2.symbol}@${temp[i].d3.symbol}" class="${deepLook3.join("") === `${temp[i].d1.symbol}${temp[i].d2.symbol}${temp[i].d3.symbol}` ? "check" : ""}">卖1修正</button>
            <button data-type="stop">取消卖1修正</button>
          </div>
          `;
        }
      }

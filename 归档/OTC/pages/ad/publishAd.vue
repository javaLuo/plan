<template>
	<div class="ad">
		<div class="title-container">
			<div class="title">{{PublishAd.title}}</div>
		</div>
		<div class="container">
		...
		<AddPayTypeModal :realName="userInfo.realName||''"
		                 ...
		                 :ok="addPayTypeHandle" />
		<GoToSetSecertPwdModal :show="setSecertPwdShow"
		                       @onClose="setSecertPwdShow = false" />
	</div>
</template>
...
import GoToSetSecertPwdModal from '~/components/Modal/goToSetsecertPwd.vue';
export default {
  components: {
    CapitalPwdModal,
    AddPayTypeModal,
    PayKYCModal,
    GoToSetSecertPwdModal,
  },
  data() {
    return {
      checked: false,
      ...
      setSecertPwdShow: false, // 提示设置资金密码弹框是否显示
      terms: {
  ...
  async mounted() {
    ...
    this.params.countryArea = this.countrayArea;

    //判断如果不是中国全部为美元 法币币种
    if (!!this.userInfo && this.userInfo.nationalEnName !== 'China') {
      this.params.payAsset = 1;
    } else {
      this.params.payAsset = 0;
    }
    this.params.adType = this.userInfo.busiType === 1 ? 0 : this.userInfo.busiType === 2 ? 1 : 0;
  ...
  computed: {
    ...
    unitPriceType() {
      ...
      if (parseInt(this.params.tradeType) === 0 && parseInt(this.params.payAsset) === 0) {
        res = Number(Utils.multi(this.averagePrice.sellPriceCny, this.params.relativePremium / 100));
      } else if (parseInt(this.params.tradeType) === 0 && parseInt(this.params.payAsset) === 1) {
        res = Number(Utils.multi(this.averagePrice.sellPriceUsd, this.params.relativePremium / 100));
      } else if (parseInt(this.params.tradeType) === 1 && parseInt(this.params.payAsset) === 0) {
        res = Number(Utils.multi(this.averagePrice.buyPriceCny, this.params.relativePremium / 100));
      } else if (parseInt(this.params.tradeType) === 1 && parseInt(this.params.payAsset) === 1) {
        res = Number(Utils.multi(this.averagePrice.buyPriceUsd, this.params.relativePremium / 100));
      }
      return res ? res.toFixed(2) : 0.0;
    },
    limitMax() {
      if (!/^\d+(\.\d{0,4})?$/.test(this.params.amount)) {
        return 0;
      }
  ...
  methods: {
    ...
    async handleCallback() {
      ...
      res = await this.createdAd(params);
      if (res && res.code === 200) {
        this.$router.push('/ad/adList');
      } else {
        this.loading = false;
        this.$message.warning(this.ErrCode[res.code] || res.code);
      }
    ...
    validateAmount(rule, value, callback) {
      ...
      } else if (parseInt(this.params.tradeType) === 0 && Number(value) > Number(this.userAmount)) {
        callback(this.PublishAd.callbackTxt[4]);

...
export default {
  ...
  async mounted() {
    ...
    history.replaceState({}, 'RightBTC', `/sliverexchange/${symbol.zi}_${symbol.mu}`);
  ...
  computed: {
    ...mapState({
      ...
      marketData: state => state.trade.marketDataSliver, // 所有交易对市场数据
    }),
    // 动态设置网页的标题
    title() {
  ...
  methods: {
    ...
    onMarketMessage(msg) {
      ...
      if (data && data.code === 200) {
        ...
        this.saveMarketData({ isS: true, data: newD });
        this.$nextTick(() => {
    ...
    // 进入页面时检测URL中的交易对是否存在
    async checkSymbol() {
      let symbol = { mu: Configs.DEFAULT.smu, zi: Configs.DEFAULT.szi };
      ...
      if (this.marketData && this.marketData.length) {
        ...
        await this.selectGroup({ isS: true }).then(res => {
          if (res && res.code === 200) {
